...
void ConvectionFvmWorker::assemble_nu(){
	...
	// TODO: add les
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	size_t sizeF = _grid.n_faces();
	_nu_velocity.resize(sizeF, 0.0);
	_nu_temperature.resize(sizeF, 0.0);
	std::vector<Vector> grad_u = _grad_computer.compute(_u);
	std::vector<Vector> grad_v = _grad_computer.compute(_v);
	double Cs = 0.173;
	for (size_t iface = 0; iface < sizeF; iface++)
	{
		size_t ci = _grid.tab_face_cell(iface)[0];
		size_t cj = _grid.tab_face_cell(iface)[1];
		if (ci == INVALID_INDEX || cj == INVALID_INDEX)
		{
			_nu_velocity[iface] = 1.0 / _Re;
			_nu_temperature[iface] = 1.0 / _Pe;
		}
		else {
			double h1 = sqrt(_grid.cell_volume(ci));
			double h2 = sqrt(_grid.cell_volume(cj));

			double dudx1 = grad_u[ci].x();
			double dudy1 = grad_u[ci].y();
			double dvdx1 = grad_v[ci].x();
			double dvdy1 = grad_v[ci].y();

			double dudx2 = grad_u[cj].x();
			double dudy2 = grad_u[cj].y();
			double dvdx2 = grad_v[cj].x();
			double dvdy2 = grad_v[cj].y();

			double nu1 = (Cs * h1) * (Cs * h1) * sqrt(2.0 * (dudx1 * dudx1 + dvdy1 * dvdy1 + 2.0 * dudy1 * dudy1));
			double nu2 = (Cs * h2) * (Cs * h2) * sqrt(2.0 * (dudx2 * dudx2 + dvdy2 * dvdy2 + 2.0 * dudy2 * dudy2));
			
			_nu_velocity[iface] = 1.0 / _Re + (nu1 + nu2) / 2.0;
			_nu_temperature[iface] = 1.0 / _Pe + (nu1 + nu2) / 2.0;
		}
	}
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
}
...
TEST_CASE("Convection, FVM-SIMPLE algorithm", "[convection-fvm]"){
	...
	RegularGrid2D grid(0, L, 0, 1, 15*L, 15);
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	//std::string grid_fn = test_directory_file("pebigrid200.vtk");
	//UnstructuredGrid2D grid = UnstructuredGrid2D::vtk_read(grid_fn);
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	ConvectionFvmWorker worker(grid, Re, Pe, E);
	worker.initialize_saver("convection-fvm-les");
	...
	for (it=1; it < max_it; ++it){
		...
		if (it % 20 == 0) worker.save_current_fields(it);
