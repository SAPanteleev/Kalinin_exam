...
namespace
{
	...
	Cavity2DFvmSimpleWorker::Cavity2DFvmSimpleWorker(const IGrid &grid, double Re, double E) : _grid(grid),
																							   _Re(Re),
		...
		const std::vector<double> &v_star)
	{
		...
		for (size_t iface = 0; iface < _grid.n_faces(); ++iface)
		{
			...
			else
			{
				...
				ret[iface] = 0.5 * (ustar_i + ustar_j) + 0.5 * _tau * (dpdn_i + dpdn_j - 2 * dpdn_ij);
			}
		}
...
{
	std::cout << std::endl
			  << "--- cfd24_test [cavity-fvm-rans-laminar] --- " << std::endl;
...
namespace
{
	...
	Cavity2DFvmSimpleWorker_KEps::Cavity2DFvmSimpleWorker_KEps(const IGrid &grid, double Re, double E) : _grid(grid),
																										 _Re(Re),
																										 _tau(compute_tau(grid, Re, E)),
																										 _alpha_p(1.0 / (1.0 + E)),
																										 _collocations(grid),
																										 ...
																										 _grad_computer(grid, _collocations)
	{
		...
		_nuT = std::vector<double>(vec_size(), delta_nuT/_Re);
	...
	void Cavity2DFvmSimpleWorker_KEps::gather_boundary_collocations()
	{
		...
		for (size_t icolloc : bi.bnd_colloc)
		{
			...
			if (icell == INVALID_INDEX)
				icell = _grid.tab_face_cell(iface)[1];
	...
	std::vector<double> Cavity2DFvmSimpleWorker_KEps::
	compute_nueff_face(double sigma) const
	{
		// nueff_face = 1/Re  + nuT_face / sigma
		// return std::vector<double>(_grid.n_faces(), 1.0 / _Re);
		std::vector<double> nueff(_grid.n_faces());
		for (size_t iface = 0; iface < _grid.n_faces(); ++iface)
		{
			size_t ci = _grid.tab_face_cell(iface)[0];
			size_t cj = _grid.tab_face_cell(iface)[1];
			...
			if (ci != INVALID_INDEX && cj != INVALID_INDEX)
			{
				nuT_face = 0.5 * (_nuT[ci] + _nuT[cj]);
			}
			else if (ci != INVALID_INDEX)
			{
				nuT_face = _nuT[ci];
			}
			else if (cj != INVALID_INDEX)
			{
				nuT_face = _nuT[cj];
			}
			nueff[iface] = 1.0 / _Re + nuT_face / sigma;
		}
		return nueff;
	}
	...
	{
		//
		std::vector<double> G(vec_size(), 0.0);
		...
		for (size_t icell = 0; icell < _grid.n_cells(); ++icell)
		{
			...
			double dudx = grad_u[icell].x();
			double dudy = grad_u[icell].y();
			double dvdx = grad_v[icell].x();
			double dvdy = grad_v[icell].y();

			// Calculate strain rate tensor components
			double S11 = dudx;
			double S12 = dudy + dvdx;
			double S22 = dvdy;

			// Calculate strain rate magnitude (2SijSij)
			double S_mag_sq = 2 * (S11 * S11 + 0.5 * S12 * S12 + S22 * S22);

			// Compute production term G = nuT * (2SijSij)
			G[icell] = _nuT[icell] * S_mag_sq;
		}
		return G;
	};
	...
	{
		//
		std::vector<double> gamma(vec_size());
		for (size_t i = 0; i < vec_size(); ++i)
		{
			gamma[i] = _Cmu * _k[i]/_nuT[i] ;
		}
		return gamma;
	}
	...
	{
		//
		std::vector<double> nuT(vec_size());
		double nu = 1/_Re;
		double l = 0.0;
		for (size_t i = 0; i < vec_size(); ++i)
		{
			// 
			l = std::min(1.0, _Cmu*std::pow(_k[i], 3/2)/_epsilon[i]);
			nuT[i] = std::max(delta_nuT*nu, l*std::sqrt(_k[i]));
		}
		return nuT;
	}
	...
	std::vector<double> Cavity2DFvmSimpleWorker_KEps::compute_epsilon(const std::vector<double> &G, const std::vector<double> &gamma) const
	{
		...
		CsrMatrix mat = assemble_convdiff_lhs(_epsilon, nu, a);
		...
		for (const auto &it : _boundary_info.bnd_cell_to_colloc)
		{
			...
			Point cellCenter = _grid.cell_center(icell);
			std::vector<size_t> boundaryFaces = _grid.boundary_faces();
			double y = 1e5;
			for (size_t iface : boundaryFaces) {
				Point faceCenter = _grid.face_center(iface);
			
				double distance = std::sqrt(
					std::pow(faceCenter.x() - cellCenter.x(), 2) +
					std::pow(faceCenter.y() - cellCenter.y(), 2) +
					std::pow(faceCenter.z() - cellCenter.z(), 2)
				);
			...
			double y_plus = y * std::sqrt(std::sqrt(_Cmu)*_k[icell]) * _Re;
			double kappa = 0.41;

			for (size_t icolloc : it.second)
			{
				double eps_i = (y_plus < 11.25) ? 2.0*_k[icell]/y/y/_Re : std::pow(_Cmu, 0.75) * std::pow(_k[icell], 3/2) / (kappa * y);
				eps_i = std::max(eps_i, 1e-6);  // ensure non-zero
				// std::cout << eps_i << std::endl;
				// on boundary
				mat.set_unit_row(icolloc);
				rhs[icolloc] = eps_i;
