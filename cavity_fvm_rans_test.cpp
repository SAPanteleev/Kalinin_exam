...
struct Cavity2DFvmSimpleWorker_KOmega{
	...
	~Cavity2DFvmSimpleWorker_KOmega();
	void initialize_saver(std::string stem);
	double step();
	void save_current_fields(size_t iter);
...
Cavity2DFvmSimpleWorker_KOmega::~Cavity2DFvmSimpleWorker_KOmega()
{
	outfile.close();
}
...
double Cavity2DFvmSimpleWorker_KOmega::step(){
	...
	// comput and out turbulent volumn
	outfile<<compute_S()<<std::endl;
...
std::vector<double> Cavity2DFvmSimpleWorker_KOmega::compute_nuT() const{
	...
	{
		// 
		l = std::min(1.0, std::pow(_k[i], 1/2)/_omega[i]);
		nuT[i] = std::max(_delta_nuT*nu, l*std::sqrt(_k[i]));
	}
	return nuT;
}
...
std::vector<double> Cavity2DFvmSimpleWorker_KOmega::compute_omega(const std::vector<double>& G, const std::vector<double>& omega) const{
	...
	for (const auto& it: _boundary_info.bnd_cell_to_colloc){
		...
		for (size_t icolloc: it.second){
			//size_t iface = _collocations.face_index(icolloc);
			double omega_i = (y_plus<11.25) ? 6.0/_Re/_beta/y/y : pow(_k[icolloc], 1.0/2.0)/kappa/y/pow(_Cmu, 1.0/4.0); // TODO
			// on boundary
			mat.set_unit_row(icolloc);
			rhs[icolloc] = omega_i;
			// on near-boundary cell
...
double Cavity2DFvmSimpleWorker_KOmega::compute_S() const
{
	double S = 0.0;
	for (size_t icell = 0; icell < _grid.n_cells(); ++icell)
	{
		S += _grid.cell_volume(icell)*_k[icell];
	}
...
TEST_CASE("Cavity 2D with k-omega model, FVM-SIMPLE-Laminar algorithm", "[cavity-fvm-rans-ko]"){
	...
	std::cout<<worker.compute_S()<<std::endl;
	CHECK(it == 21);
}
